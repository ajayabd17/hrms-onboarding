    ...
    try:
        ...
        if state_machine_arn:
            exec_resp = sfn.start_execution(
                ...
                input=json.dumps({'employeeId': employee_id, 'email': email})
            )
        ...
        return _resp(400, {'error': f'cognito invalid parameter: {str(exc)}'}, event)
    except cognito.exceptions.UsernameExistsException:
        return _resp(409, {'error': 'employee email already exists in Cognito'}, event)
